...
fn main() {
    ...
    let test_cases = vec![
        ...
        ("interval(3, [date 2024-12-25, time 12:00..13:00])", "Skip Christmas & Lunch"),
        ("at(09:30, [weekday 6, time 22:00..06:00])", "Skip Saturday & Night"),
        
        // Complex scenarios
        ("wait(1, [weekday 1, weekday 2, weekday 3])", "Skip Mon/Tue/Wed"),
        ("interval(2, [date 2024-12-25, date 2024-01-01, time 12:00..13:00])", "Skip Holidays & Lunch"),
    ];
    ...
    for (task_str, description) in error_cases {
        match Task::parse(task_str) {
            Ok(task) => {
                println!("⚠️  '{}' ({}) -> Should have failed but got: {}", task_str, description, task);
            }
            Err(err) => {
                println!("✅ '{}' ({}) -> Expected error: {}", task_str, description, err);
            }
        }
    }
